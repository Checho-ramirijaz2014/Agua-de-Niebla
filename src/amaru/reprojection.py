...
def reproject_goes16(input_file: str, output_name: str) -> None:
    """
    ...
    output_file = f"{base_name}{output_name}.nc"
        ...
        with netCDF4.Dataset(output_file, "w", format="NETCDF3_CLASSIC") as dst:
            ...
            for attr_name in src.ncattrs():
                try:
                    ...
                    print(
                        f"Warning: Could not copy global attribute '{attr_name}': {e}"
                    )
                    continue
            ...
            for var_name, var in src.variables.items():
                ...
                for attr_name in var.ncattrs():
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Could not copy attribute '{attr_name}' for variable '{var_name}': {e}"
                        )
                        continue
                ...
                try:
                    ...
                    print(
                        f"Warning: Could not copy data for variable '{var_name}': {e}"
                    )
                    continue
...
def update_metadata(output_file):
    ...
    try:
        ...
        with netCDF4.Dataset(output_file, "r+", format="NETCDF3_CLASSIC") as nc:
            ...
            nc.setncattr(
                "title", "ABI L2+ Cloud and Moisture Imagery reflectance factor"
            )
            ...
            ogc_wkt = (
                'GEOGCS["NAD83", DATUM["NAD83", SPHEROID["GRS_1980", 6378137.0, '
            ...
            history = (
                f"Exported to NetCDF-3 CF-1.0 conventions by the NOAA Weather and "
...
def display_info(filename):
    ...
    try:
        ...
        for i in range(dataset.RasterCount):
            ...
            print(
                f"Band {i+1} Block={band.GetBlockSize()[0]}x{band.GetBlockSize()[1]} "
...
def goes_clip(
    ...
    with mp.Pool(cpu) as pool:
        pool.starmap(
            reproject_goes16,
            [(file, out_name) for file in files],
        )
    ...
    if remove_unclipped:
        for file in files:
            try:
                ...
                raise RuntimeError(
                    f"File {file} not found. An error ocurred when reprojecting goes"
                )
