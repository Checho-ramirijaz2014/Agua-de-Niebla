...
coordenates = np.array([[-85, -60], [-10, -40]])  # longitude = x  # latitude = y
...
def convertir_radianes_a_grados(vector: np.array):
    ...
    phi_c = np.arctan(
        (r_pol / r_eq) * (r_pol / r_eq) * np.tan(y_coordinate)
    )  # geocentric latitude

    r_c = r_pol / (
        np.sqrt(1 - E * E * np.cos(phi_c) * np.cos(phi_c))
    )  # geocentric distance to the point on ellipsoid
